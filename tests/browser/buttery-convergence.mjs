...
async function main() {
  ...
  debugEmits.length = 0;
  const emitted = await cdp.eval(`window.crownDebug.emit('crown-layout', 'browser-redaction-proof', { phase:'seated', safeField:'survives', token:'browser-sentinel', nested:{ pin:'browser-sentinel', body:'browser-sentinel', requestBody:'browser-sentinel', payload:'browser-sentinel', headers:{ authorization:'browser-sentinel' }, capability:'browser-sentinel', connectionString:'browser-sentinel', localStorage:'browser-sentinel', domSnapshot:'browser-sentinel', sourceHtml:'browser-sentinel' }, array:[{ secret:'browser-sentinel' }, { safeNested:'survives' }] })`);
