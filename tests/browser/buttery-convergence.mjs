...
async function main() {
  ...
  const guestDnsProjection = await cdp.eval(`(() => ({ tab: document.querySelectorAll('[data-pane="unbound"]').length, host: document.querySelectorAll('[data-pane-panel="unbound"], [data-view-panel="unbound"]').length }))()`);
  assert('guest_projection_omits_admin_dns_host', guestDnsProjection.tab === 0 && guestDnsProjection.host === 0, JSON.stringify(guestDnsProjection));
  ...
  const admittedDnsHost = await eventually('admin DNS host reconciled', () => cdp.eval(`(() => ({ tab: document.querySelectorAll('[data-pane="unbound"]').length, host: document.querySelectorAll('[data-pane-panel="unbound"][data-view-panel="unbound"][data-admitted-pane-host="true"]').length }))()`).then(state => state.tab === 1 && state.host === 1 ? state : false));
  assert('admin_tab_projection_reconciles_one_dns_host_before_activation', admittedDnsHost.tab === 1 && admittedDnsHost.host === 1, JSON.stringify(admittedDnsHost));
  const dnsAdmission = await cdp.eval(`(async () => { await showPane('unbound', { refresh: true }); return { state: window.getImmortalFloorState(), fault: document.documentElement.dataset.immortalFloorFault || '', active: [...document.querySelectorAll('[data-pane-panel].active')].map(node => node.dataset.panePanel), hosts: document.querySelectorAll('[data-pane-panel="unbound"][data-view-panel="unbound"]').length }; })()`);
  assert('admin_dns_activation_seats_without_guest_missing', dnsAdmission.state === 'Seated' && dnsAdmission.fault !== 'guest-missing' && dnsAdmission.active.length === 1 && dnsAdmission.active[0] === 'unbound' && dnsAdmission.hosts === 1, JSON.stringify(dnsAdmission));
  const repeatedDnsProjection = await cdp.eval(`(async () => { await refreshTabBar('unbound'); await refreshTabBar('unbound'); return document.querySelectorAll('[data-pane-panel="unbound"][data-view-panel="unbound"]').length; })()`);
  assert('repeated_admin_projection_does_not_duplicate_dns_host', repeatedDnsProjection === 1, String(repeatedDnsProjection));
  await cdp.eval(`showPane('stats', { refresh: true })`);
