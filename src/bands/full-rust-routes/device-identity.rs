...
fn device_identity_read(path: &str, caduceus_path: &str, headers: &axum::http::HeaderMap) -> Response {
    if let Some(refusal) = device_identity_admin(headers, path) { return refusal; }
    let readback = caduceus_http("GET", caduceus_path);
    if readback.ok { return (StatusCode::OK, Json(readback.body)).into_response(); }
...
async fn device_boundary_route(headers: axum::http::HeaderMap) -> Response {
    device_identity_read("/api/network/dhcp/boundary", "/api/v1/network/dhcp/boundary", &headers)
}
async fn device_leases_route(headers: axum::http::HeaderMap) -> Response {
    device_identity_read("/api/network/dhcp/leases", "/api/v1/network/dhcp/leases", &headers)
}
async fn device_reservations_route(headers: axum::http::HeaderMap) -> Response {
    device_identity_read("/api/network/dhcp/reservations", "/api/v1/network/dhcp/reservations", &headers)
}
async fn device_dns_read_route(headers: axum::http::HeaderMap) -> Response {
    device_identity_read("/api/network/dns/read", "/api/v1/network/dns/read", &headers)
}

async fn device_claim_route(headers: axum::http::HeaderMap, payload: Option<Json<serde_json::Value>>) -> Response {
    let path = "/api/network/device/claim";
    if let Some(refusal) = device_identity_admin(&headers, path) { return refusal; }
    // One identity binding is one Caduceus call: DHCP and DNS stay coordinated by staff.
    let readback = caduceus_http_json("POST", "/api/v1/network/device/claim", payload.map(|Json(v)| v).unwrap_or_else(|| serde_json::json!({})), None);
    if readback.ok { return (StatusCode::OK, Json(readback.body)).into_response(); }
