fn shell_dhcp_client() -> &'static str {
    ...
    function renderDhcp() {
      ...
      set('[data-dhcp-homeserver]', stats.homeserver_ip ?? stats.homeserverIp ?? '192.168.123.1');
      set('[data-dhcp-reservations-count]', `${reservationCount}/${reservationTotal}`);
      set('[data-dhcp-hosts-count]', reservationCount + leaseCount);
      set('[data-dhcp-leases-count]', `${leaseCount}/${leaseTotal}`);
    ...
    async function hydrateDhcp() {
      ...
      try {
        ...
        if (slider) {
          ...
          setDhcpBoundaryLabel(slider.value);
        }
        renderDhcp();
      } catch (failure) { if (error) { error.hidden = false; error.textContent = `DHCP unavailable: ${failure.message}`; } const loading = document.querySelector('[data-dhcp-loading]'); if (loading) loading.hidden = true; }
    }
    function setDhcpBoundaryLabel(value) { const label = document.querySelector('[data-dhcp-boundary-value]'); if (label) label.textContent = value; }
    async function dhcpMutate(method, route, body) {
      await dhcpJson(route, { method, body: body === undefined ? undefined : JSON.stringify(body) });
      await hydrateDhcp();
    }
    document.querySelector('[data-dhcp-refresh]')?.addEventListener('click', hydrateDhcp);
    document.querySelector('[data-dhcp-anonymize]')?.addEventListener('change', event => { dhcpState.anonymized = event.currentTarget.checked; renderDhcp(); });
    document.querySelector('[data-dhcp-boundary]')?.addEventListener('input', event => setDhcpBoundaryLabel(event.currentTarget.value));
    document.querySelector('[data-dhcp-boundary-save]')?.addEventListener('click', () => { const slider = document.querySelector('[data-dhcp-boundary]'); if (slider) dhcpMutate('POST', '/api/dhcp/pool-boundary', { maxReservations: Number(slider.value) }); });
    document.querySelector('[data-dhcp-add-form]')?.addEventListener('submit', async event => { event.preventDefault(); const data = new FormData(event.currentTarget); const payload = { mac: data.get('mac'), hostname: data.get('hostname') || undefined }; if (data.get('ip')) payload.ip = data.get('ip'); await dhcpMutate('POST', '/api/dhcp/reservations', payload); event.currentTarget.reset(); });
    document.querySelector('[data-dhcp-items]')?.addEventListener('click', async event => {
      const pin = event.target.closest('[data-dhcp-pin]'); if (pin) return dhcpMutate('POST', '/api/dhcp/reservations', { mac: pin.dataset.dhcpPin, hostname: pin.dataset.dhcpHostname || undefined });
      const remove = event.target.closest('[data-dhcp-remove]'); if (remove) return dhcpMutate('DELETE', '/api/dhcp/reservations/' + encodeURIComponent(remove.dataset.dhcpRemove));
      const edit = event.target.closest('[data-dhcp-edit]'); if (edit) { const ip = prompt('New reserved IP address', edit.dataset.dhcpIp || ''); if (ip) return dhcpMutate('PUT', '/api/dhcp/reservations/' + encodeURIComponent(edit.dataset.dhcpEdit), { ip }); }
    });
