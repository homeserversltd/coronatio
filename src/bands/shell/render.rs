...
const DNS_PANE: &str = r#"<section class="pane" id="pane-unbound" data-pane-panel="unbound" data-view-panel="unbound" role="tabpanel" aria-label="DNS"><article class="dns-tablet" data-dns-tablet data-admin-only="true" data-admin-viewport="unbound"><header class="dns-tablet-header"><div><h2>Local DNS</h2><p>Device names are read-only projections; claim changes the device binding.</p></div><button type="button" class="ui-button ui-button--secondary ui-button--small" data-dns-refresh>Refresh</button></header><p data-dns-state aria-live="polite">Ready to read local records.</p><section class="identity-roster-host" data-identity-roster-host="dns" data-identity-perspective="name"><h3>Devices</h3><div data-identity-roster></div></section><section class="dns-records" data-dns-names></section></article></section>"#;
