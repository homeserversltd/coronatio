...
const AGENT_SERVICE_LOCKOUT_FAILURES: u8 = 5;
const AGENT_SERVICE_LOCKOUT_DURATION_SECS: u64 = 24 * 60 * 60;
const AGENT_SERVICE_LOCKOUT_PEER_CAP: usize = 1024;
const AGENT_SERVICE_LOCKOUT_STATE_SCHEMA: &str = "coronatio.exousia.agent.lockout.v1";
const AGENT_SERVICE_LOCKOUT_STATE_FAILURE: &str = "coronatio-exousia-agent-lockout-state-unavailable";
const AGENT_SERVICE_LOCKED_OUT: &str = "coronatio-exousia-agent-pin-locked-out";
const AGENT_SERVICE_PIN_FAILURE_EVENT: &str = "exousia-agent-pin-failure";
const AGENT_SERVICE_LOCKOUT_EVENT: &str = "exousia-agent-pin-lockout";

#[derive(Debug, Clone, Serialize, Deserialize, Default)]
struct AgentServiceLockoutPeer {
    #[serde(default)]
    consecutive_failures: u8,
    #[serde(default)]
    lockout_deadline_unix_seconds: Option<u64>,
    #[serde(default)]
    last_seen_unix_seconds: u64,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct AgentServiceLockoutState {
    schema: String,
    #[serde(default)]
    peers: BTreeMap<std::net::IpAddr, AgentServiceLockoutPeer>,
}

impl Default for AgentServiceLockoutState {
    fn default() -> Self {
        Self { schema: AGENT_SERVICE_LOCKOUT_STATE_SCHEMA.to_string(), peers: BTreeMap::new() }
    }
}

#[derive(Debug, Clone, Copy)]
struct AgentServiceLockoutStatus {
    consecutive_failures: u8,
    lockout_deadline_unix_seconds: Option<u64>,
}

struct AgentServiceLockoutStore {
    path: Option<PathBuf>,
    state: AgentServiceLockoutState,
}

impl Default for AgentServiceLockoutStore {
    fn default() -> Self {
        Self { path: None, state: AgentServiceLockoutState::default() }
    }
}

impl AgentServiceLockoutStore {
    fn reload(&mut self, path: &std::path::Path) -> Result<(), ()> {
        let state = match std::fs::read_to_string(path) {
            Ok(raw) => serde_json::from_str::<AgentServiceLockoutState>(&raw).map_err(|_| ())?,
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => AgentServiceLockoutState::default(),
            Err(_) => return Err(()),
        };
        if state.schema != AGENT_SERVICE_LOCKOUT_STATE_SCHEMA {
            return Err(());
        }
        if state.peers.values().any(|entry| {
            entry.consecutive_failures > AGENT_SERVICE_LOCKOUT_FAILURES
                || (entry.lockout_deadline_unix_seconds.is_some()
                    && entry.consecutive_failures < AGENT_SERVICE_LOCKOUT_FAILURES)
        }) {
            return Err(());
        }
    ...
    fn trim_to_cap(&mut self) {
        while self.state.peers.len() > AGENT_SERVICE_LOCKOUT_PEER_CAP {
            let oldest = self
                .state
                .peers
                .iter()
                .min_by_key(|(peer, entry)| {
                    (
                        entry.lockout_deadline_unix_seconds.is_some(),
    ...
    fn blocked(&mut self, peer: std::net::IpAddr, now: u64) -> Option<AgentServiceLockoutStatus> {
        let entry = self.state.peers.get_mut(&peer)?;
        entry.last_seen_unix_seconds = now;
        match entry.lockout_deadline_unix_seconds {
            Some(deadline) if deadline > now => Some(AgentServiceLockoutStatus {
                consecutive_failures: entry.consecutive_failures,
                lockout_deadline_unix_seconds: Some(deadline),
            }),
            Some(_) => {
                entry.consecutive_failures = 0;
                entry.lockout_deadline_unix_seconds = None;
                None
            },
            None => None,
        }
    ...
    fn record_wrong_pin(&mut self, peer: std::net::IpAddr, now: u64) -> Result<AgentServiceLockoutStatus, ()> {
        if self.blocked(peer, now).is_some() {
            return Err(());
        }
        if !self.state.peers.contains_key(&peer) && self.state.peers.len() >= AGENT_SERVICE_LOCKOUT_PEER_CAP {
            self.trim_to_cap();
            if self.state.peers.len() >= AGENT_SERVICE_LOCKOUT_PEER_CAP {
                let oldest = self
                    .state
                    .peers
                    .iter()
                    ...
                    .ok_or(())?;
                self.state.peers.remove(&oldest);
            }
        }
        let entry = self.state.peers.entry(peer).or_default();
        entry.consecutive_failures = entry.consecutive_failures.saturating_add(1).min(AGENT_SERVICE_LOCKOUT_FAILURES);
        entry.last_seen_unix_seconds = now;
        if entry.consecutive_failures >= AGENT_SERVICE_LOCKOUT_FAILURES {
            entry.lockout_deadline_unix_seconds = Some(now.saturating_add(AGENT_SERVICE_LOCKOUT_DURATION_SECS));
        }
        let status = AgentServiceLockoutStatus {
            consecutive_failures: entry.consecutive_failures,
            lockout_deadline_unix_seconds: entry.lockout_deadline_unix_seconds,
        };
    ...
    fn reset_peer(&mut self, peer: std::net::IpAddr) -> Result<(), ()> {
...
static AGENT_SERVICE_LOCKOUT_STORE: OnceLock<Mutex<AgentServiceLockoutStore>> = OnceLock::new();

fn agent_service_lockout_path() -> PathBuf {
    env::var_os("CORONATIO_EXOUSIA_AGENT_LOCKOUT_PATH")
        .filter(|value| !value.is_empty())
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("/var/lib/coronatio/exousia-agent-lockouts.json"))
}

fn agent_service_now() -> u64 {
    SystemTime::now().duration_since(UNIX_EPOCH).map(|duration| duration.as_secs()).unwrap_or(0)
}

fn reflect_agent_lockout_event(
    kind: &'static str,
    peer: std::net::IpAddr,
    status: AgentServiceLockoutStatus,
) {
    caduceus_hyalos_reflect_best_effort(
        kind,
        "warn".to_string(),
        if kind == AGENT_SERVICE_LOCKOUT_EVENT {
            "admin lockout blocked".to_string()
        } else {
            "admin authentication failure".to_string()
        },
        Some(peer.to_string()),
        Some(serde_json::json!({
            "phase": "exousia-agent-pin",
            "source_ip": peer.to_string(),
            "consecutive_failures": status.consecutive_failures,
            "lockout_deadline_unix_seconds": status.lockout_deadline_unix_seconds,
        })),
    );
}
...
async fn caduceus_agent_service_route(
    ...
    let peer = connect_info.0.ip();
    let mut lockouts = AGENT_SERVICE_LOCKOUT_STORE
        .get_or_init(|| Mutex::new(AgentServiceLockoutStore::default()))
        .lock()
        .expect("agent service lockout store lock");
    if lockouts.reload(&agent_service_lockout_path()).is_err() {
        return agent_service_refusal(
            StatusCode::SERVICE_UNAVAILABLE,
            AGENT_SERVICE_LOCKOUT_STATE_FAILURE,
            Some(service),
            Some(action),
            None,
        );
    }
    let now = agent_service_now();
    if let Some(status) = lockouts.blocked(peer, now) {
        reflect_agent_lockout_event(AGENT_SERVICE_LOCKOUT_EVENT, peer, status);
        return agent_service_refusal(
            StatusCode::TOO_MANY_REQUESTS,
            AGENT_SERVICE_LOCKED_OUT,
            Some(service),
            Some(action),
            None,
        );
    ...
    let Some(attendance) = opened.proof else {
        if opened.receipt.code == "caduceus-attendance-pin-wrong" {
            let status = match lockouts.record_wrong_pin(peer, now) {
                Ok(status) => status,
                Err(_) => {
                    let failure = AgentServiceLockoutStatus { consecutive_failures: 0, lockout_deadline_unix_seconds: None };
                    reflect_agent_lockout_event(AGENT_SERVICE_PIN_FAILURE_EVENT, peer, failure);
                    return agent_service_refusal(
                        StatusCode::SERVICE_UNAVAILABLE,
                        AGENT_SERVICE_LOCKOUT_STATE_FAILURE,
                        Some(service),
            ...
            reflect_agent_lockout_event(AGENT_SERVICE_PIN_FAILURE_EVENT, peer, status);
        }
        return agent_service_refusal(
    ...
    if lockouts.reset_peer(peer).is_err() {
